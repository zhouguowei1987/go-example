...
const (
	NhcEnableHttpProxy = false
	NhcHttpProxyUrl    = "111.225.152.186:8089"
)

func NhcSetHttpProxy() (httpclient *http.Client) {
	ProxyURL, _ := url.Parse(NhcHttpProxyUrl)
...
// ychEduSpider 获取卫生标准文档
// @Title 获取卫生标准文档
// @Description http://www.nhc.gov.cn/，获取卫生标准文档
func main() {
	// 第一步获取所有大分类
	categoryIndexRequestUrl := "http://www.nhc.gov.cn/ywgz/fgbz/bz/bzwb/"
	...
	for _, liNode := range stBzCBaseTabUlLiNodes {
		...
		categoryRequestUrl = strings.ReplaceAll(categoryRequestUrl, "/", "")
		categoryRequestUrl = "http://www.nhc.gov.cn/ywgz/fgbz/bz/bzwb/" + categoryRequestUrl + "/"
		...
		if len(smallCategoryRequestUrls) > 0 {
			for _, smallCategoryUrl := range smallCategoryRequestUrls {
				...
				for smallCategoryPage := 0; smallCategoryPage < smallCategoryMaxPage; smallCategoryPage++ {
					...
					smallCategoryDoc, err := NhcBzList(smallCategoryRequestUrl, referer)
					if err != nil {
						fmt.Println(err)
						continue
					}
							...
							if len(bzDetailANodes) > 0 {
								for _, bzDetailANode := range bzDetailANodes {
									...
									if strings.Contains(bzDownloadHref, ".pdf") {
										...
										// 查看bzDownloadHref是否含有www.nhc.gov.cn
										if !strings.Contains(bzDownloadHref, "www.nhc.gov.cn") {
											// 不含有www.nhc.gov.cn，下载连接需要处理
										...
										if _, err := os.Stat(filePath); err != nil {
											// 开始下载
											fmt.Println("=======开始下载========")
											err = downloadNhc(downLoadUrl, bzDetailRequestUrl, filePath)
											if err != nil {
												fmt.Println(err)
												continue
											}
...
func NhcBzList(requestUrl string, referer string) (doc *html.Node, err error) {
	// 初始化客户端
	...
	if NhcEnableHttpProxy {
		client = NhcSetHttpProxy()
	}
	...
	req.Header.Set("Origin", "http://www.nhc.gov.cn/")
...
func downloadNhc(attachmentUrl string, referer string, filePath string) error {
	// 初始化客户端
	var client *http.Client = &http.Client{
		Transport: &http.Transport{
			Dial: func(netw, addr string) (net.Conn, error) {
