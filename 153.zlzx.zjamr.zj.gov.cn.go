	...
	for isPageListGo {
		...
		if len(aNodes) > 0 {
			for _, aNode := range aNodes {
				...
				//复制文件
				tempFilePath := strings.ReplaceAll(filePath, "zlzx.zjamr.zj.gov.cn", "temp-dbba.sacinfo.org.cn")
