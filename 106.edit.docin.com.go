...
func main() {
	...
	for {
		...
		pageListDoc, err := RequestDocInDoc(pageListUrl, referer)
		if err != nil {
			fmt.Println(err)
			break
		}
		...
		editDoc, err := RequestDocInDoc(editUrl, referer)
		if err != nil {
			fmt.Println(err)
			break
		}
		fmt.Println(htmlquery.InnerText(editDoc))
...
func RequestDocInDoc(requestUrl string, referer string) (doc *html.Node, err error) {
	// 初始化客户端
