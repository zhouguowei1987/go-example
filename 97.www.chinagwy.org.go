	...
	for isPageListGo {
		...
		if len(dlNodes) >= 1 {
			for _, dlNode := range dlNodes {
				...
				fileName = strings.ReplaceAll(fileName, " ", "")
				fileName = strings.ReplaceAll(fileName, "及答案", "")
				fileName = strings.ReplaceAll(fileName, "及参考答案", "")
				fileName = strings.ReplaceAll(fileName, "真题精选", "试题")
				fileName = strings.ReplaceAll(fileName, "（精选）", "")
				fileName = strings.ReplaceAll(fileName, "精选真题", "试题")
