		...
		for isPageGo {
			...
			if len(divNodes) >= 1 {
				for _, divNode := range divNodes {
					...
					err = downloadYchEdu(downloadUrl, filePath, softID, title)
					if err != nil {
						fmt.Println(err)
					}
				}
...
func downloadYchEdu(attachmentUrl string, filePath string, softID string, title string) error {
	// 初始化客户端
	...
	out, err := os.Create(filePath + softID + "-" + title + suffix)
	if err != nil {
		return err
	}
	defer out.Close()
