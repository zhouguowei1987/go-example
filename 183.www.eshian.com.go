...
func main() {
	...
	for id := startId; id <= endId; id++ {
		...
		titleNode := htmlquery.FindOne(detailDoc, `//html/body/div[2]/div[3]/div[1]/strong`)
		if titleNode == nil {
		...
		regDownloadViewUrl := regexp.MustCompile(`var url = "/pdf/web/viewer.html\?filePath=(.*?)";`)
		regDownloadViewUrlMatch := regDownloadViewUrl.FindAllSubmatch([]byte(htmlquery.InnerText(detailDoc)), -1)
...
func EShiAnDetailDoc(url string) (doc *html.Node, err error) {
	...
	req.Header.Set("Cookie", EShiAnCookie)
	req.Header.Set("Content-Type", "text/html; charset=utf-8")
	req.Header.Set("Host", "www.eshian.com")
	req.Header.Set("Referer", "https://www.eshian.com/sat/standard/standardlist/0")
...
func downloadEShiAn(requestUrl string, referer string, filePath string) error {
	...
	req.Header.Set("Cookie", EShiAnCookie)
	req.Header.Set("Host", "www.eshian.com")
	req.Header.Set("Origin", "https://www.eshian.com")
...
func copyEShiAnFile(src, dst string) (err error) {
	in, err := os.Open(src)
	if err != nil {
		return err
	}
